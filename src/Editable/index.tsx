...
export interface EditableTableProps<R = any> {
  sortMode?: 'popover' | false; // 拖动排序方式，拖拽手柄排序，序列号气泡输入排序；默认关闭
...
const EditableTable = <R extends {}>(props: EditableTableProps<R>) => {
  ...
  const showAddBtn = !readonly && !hideAddBtn && (!max || max > state.length);
  ...
  const getColumnByField: getColumnByFieldType = useCallback(
    col => {
  ...
  const columns: ColumnsType<any> = useMemo(() => {
    ...
    return list;
  }, [sortMode, fields, multiple, readonly]);
