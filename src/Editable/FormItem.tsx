...
const Error = (props: any) => {
  const hasError = !!props.errors?.length;
  ...
  if (React.isValidElement(props.children)) {
    children = React.cloneElement(props.children, {
      ...
      onMouseLeave: (e: any) => {
        ...
        if (typeof props.children.props?.onMouseLeave === 'function') {
    ...
    <div className={hasError ? 'ant-form-item-has-error' : ''}>
      <Popover
        ...
        content={
          ...
          >
            {props.errors.map((str: React.ReactNode, index: number) => {
              return (
...
const FormItem: React.FC<InternalFieldProps> = props => {
